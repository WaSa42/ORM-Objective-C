...
- (instancetype)init {
    ...
    if (self) {
        ...
        if ((code = sqlite3_open([self.finalPath UTF8String], &database)) != SQLITE_OK) {
            [NSException raise:@"Invalid database filename" format:@"Error %d: %s", code, sqlite3_errmsg(database)];
        }
...
- (void)execute:(NSString *)query {
    ...
    // Check the query
    if((code = sqlite3_prepare_v2(database, [query UTF8String], -1, &stmt, NULL)) != SQLITE_OK) {
        [NSException raise:@"Invalid query" format:@"Error %d: %s", code, sqlite3_errmsg(database)];
    }

    // Execute the query
    if((code = sqlite3_step(stmt)) != SQLITE_DONE) {
        [NSException raise:@"Invalid request" format:@"Error %d: %s", code, sqlite3_errmsg(database)];
    }
